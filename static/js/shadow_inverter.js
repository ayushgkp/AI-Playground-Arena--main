...
class ShadowInverterGame {
  ...
  init() {
    ...
    this.disguiseModal = new bootstrap.Modal(document.getElementById('disguiseModal'));
    ...
    if (this.dCanvas) {
        ...
        this.dCanvas.addEventListener("mouseleave", () => this.dDrawing = false);
        this.dCanvas.addEventListener("mousemove", (e) => this.drawDisguise(e));
        
        document.getElementById("btnClearDisguise").addEventListener("click", () => {
             this.dCtx.fillStyle = "#fff";
             this.dCtx.fillRect(0, 0, 300, 300);
        });
        
        document.getElementById("btnRandomDisguise").addEventListener("click", () => this.randomizeDisguise());
        
        document.getElementById("btnConfirmDisguise").addEventListener("click", () => this.generateDisguise());
    }
  ...
  async generateDisguise() {
      ...
      try {
          ...
          formData.append("prompt", "cyberpunk hologram texture, glitch art, green matrix code");
          formData.append("guidance_scale", 7.5);
          formData.append("num_steps", 20);
          
          console.log("Sending request to /api/sketch_to_image...");
          const res = await fetch("/api/sketch_to_image", { method: "POST", body: formData });
  ...
  generateLevel() {
    ...
    // Spawn Guards
    this.guards = [
        { x: 10, y: 10, path: [{x:10,y:10}, {x:15,y:10}], pathIndex: 0, alert: 0 }
    ];
  }
  ...
  tick() {
    ...
    this.guards.forEach(guard => {
        // 1. Move Guard (Patrol)
        if (this.stealth > 0) { // Only move if game not over
            ...
            if (Math.abs(guard.x - target.x) < 0.1 && Math.abs(guard.y - target.y) < 0.1) {
                guard.x = target.x;
                guard.y = target.y;
  ...
  toggleVision() {
      ...
      if (this.visionMode === "noise") {
          ...
          this.overlay.style.backgroundImage = "url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAYAAACp8Z5+AAAAIklEQVQIW2NkQAKrVq36zwjjgzj//v37zaDfv3//MzOzAABGMAvYzne41gAAAABJRU5ErkJggg==')"; // Scanline pattern
      } else {
  ...
  render() {
          ...
          for (let x = 0; x < this.GRID_SIZE; x++) {
              ...
              if (this.visionMode === "physical") {
                  ...
                  if (tile.noiseType === "wall") {
                       ...
                       this.ctx.fillText(String.fromCharCode(0x30A0 + Math.random() * 96), px + 5, py + 20);
      ...
      if (this.player.disguised && this.player.disguiseImg) {
          ...
          this.ctx.drawImage(this.player.disguiseImg, 
              this.player.x * this.TILE_SIZE, 
              this.player.y * this.TILE_SIZE, 
              this.TILE_SIZE, this.TILE_SIZE);
          this.ctx.strokeStyle = "#0f0";
          this.ctx.lineWidth = 2;
          this.ctx.strokeRect(this.player.x * this.TILE_SIZE, this.player.y * this.TILE_SIZE, this.TILE_SIZE, this.TILE_SIZE);
      } else {
          // Normal Player
          this.ctx.fillStyle = "#0ff";
          this.ctx.beginPath();
          this.ctx.arc(this.player.x * this.TILE_SIZE + this.TILE_SIZE/2, this.player.y * this.TILE_SIZE + this.TILE_SIZE/2, this.TILE_SIZE/3, 0, Math.PI*2);
          this.ctx.fill();
      }
      this.ctx.restore();
      ...
      this.ctx.fillText("YOU", this.player.x * this.TILE_SIZE + this.TILE_SIZE/2, this.player.y * this.TILE_SIZE - 5);
      
      // Draw Guards
      this.guards.forEach(g => {
          this.ctx.fillStyle = g.alert > 0 ? "#ff0" : "#f00"; // Yellow if alert
          this.ctx.fillRect(g.x * this.TILE_SIZE + 5, g.y * this.TILE_SIZE + 5, this.TILE_SIZE-10, this.TILE_SIZE-10);
          
          // Guard Label
          this.ctx.fillStyle = "#f00";
          this.ctx.font = "10px Arial";
          this.ctx.textAlign = "center";
          this.ctx.fillText("GUARD", g.x * this.TILE_SIZE + this.TILE_SIZE/2, g.y * this.TILE_SIZE - 5);
      });
  }
}
